...
public class MainActivity extends Activity {
    ...
    private static final String TARGET_URL = "http://dclub.vfq.theclub.mobi/Campaign/APP.aspx?ctg=OF06";
    ...
    private String extractOtp(String message) {
        ...
        Matcher matcher = OTP_EN_MESSAGE_PATTERN.matcher(message);
        if (matcher.find()) {
            return matcher.group(1);
        }

        matcher = OTP_AR_MESSAGE_PATTERN.matcher(message);
        if (matcher.find()) {
            return matcher.group(1);
        }

        matcher = OTP_PATTERN.matcher(message);
        if (matcher.find()) {
            return matcher.group(1);
        }
        return null;
    ...
    private void attachSubmitHook() {
        final String hookScript = "(function(){"
                ...
                + "function isOtpCandidate(el){"
                + "if(!el||!isVisible(el)){return false;}"
                + "var tag=(el.tagName||'').toLowerCase();"
                + "if(tag!=='input'&&tag!=='textarea'){return false;}"
                + "var ph=((el.placeholder||'')+'').toLowerCase().replace(/\\s+/g,'');"
                + "var id=((el.id||'')+'').toLowerCase();"
                + "var max=((el.maxLength||'')+'').toLowerCase();"
                + "if(ph==='xxxx'||max==='4'){return true;}"
                + "return id==='txtmsisdn'&&max==='4';"
                + "}"
                ...
                + "var max=((el.maxLength||'')+'').toLowerCase();"
                + "if((id==='txtmsisdn'&&max==='4')||ph==='xxxx'||max==='4'){return false;}"
                ...
                + "return false;"
                + "}"
                + "function triggerOtpPrompt(){"
                + "window.__stcksaOtpPhase=true;"
                + "if(window.__stcksaOtpPrompted){return;}"
                + "window.__stcksaOtpPrompted=true;"
                + "if(window.AndroidOtpBridge&&AndroidOtpBridge.onOtpInputFocused){AndroidOtpBridge.onOtpInputFocused();}"
                + "}"
                + "function triggerMsisdnPrompt(){"
                + "if(window.__stcksaSuppressMsisdnHint||window.__stcksaOtpPhase){return;}"
                + "if(window.__stcksaMsisdnPrompted){return;}"
                + "window.__stcksaMsisdnPrompted=true;"
                ...
                + "if(isOtpCandidate(active)){triggerOtpPrompt();return;}"
                + "if(isMsisdnCandidate(active)){triggerMsisdnPrompt();return;}"
                + "var otp=document.querySelector(\"input[placeholder='XXXX'],input[placeholder='xxxx'],input[maxlength='4']\");"
                + "if(isOtpCandidate(otp)){try{otp.focus();}catch(e){}triggerOtpPrompt();return;}"
                + "var msisdn=document.querySelector(\"input[placeholder='XXXXXXXX'],input[placeholder='xxxxxxxx'],#txtMsisdn,input[name='txtMsisdn'],input[maxlength='8'],input[type='tel']\");"
                + "if(isMsisdnCandidate(msisdn)){try{msisdn.focus();}catch(e){}triggerMsisdnPrompt();}"
                + "}"
                + "if(!window.__stcksaMsisdnGlobalBound){window.__stcksaMsisdnGlobalBound=true;"
                + "document.addEventListener('focusin',function(e){"
                + "var el=e&&e.target?e.target:null;"
                + "if(isOtpCandidate(el)){triggerOtpPrompt();return;}"
                + "if(!isMsisdnCandidate(el)){return;}"
                + "triggerMsisdnPrompt();"
                + "},true);"
                ...
                + "triggerMsisdnPrompt();"
                + "},true);"
                + "setTimeout(maybeAutoPrompt,120);"
                + "setTimeout(maybeAutoPrompt,600);"
                + "setTimeout(maybeAutoPrompt,1500);"
                + "}"
                + "})();";
    ...
    private class OtpJsBridge {
        @JavascriptInterface
        public void onOtpInputFocused() {
            runOnUiThread(() -> {
                otpFlowStarted = true;
                Log.d(TAG, "OTP field (XXXX) focused. Starting SMS consent listener.");
                startSmsUserConsent();
            });
        }
